...
export const TOOLS = [
  ...
  {
    name: "slack_subscribe_notifications",
    description: "Subscribe to real-time Slack message notifications via Claude Code channels. " +
      "By default the server sends no notifications — you must subscribe first. " +
      "Matching messages arrive as <channel source=\"slack-mcp-server\" ...> tags in real time. " +
      "Use silent_channels for channels that should queue messages without push delivery.",
    inputSchema: {
      type: "object",
      properties: {
        channels: {
          type: "array",
          items: { type: "string" },
          description: "Channel IDs to receive notifications for"
        },
        dms: {
          type: "boolean",
          description: "Subscribe to all direct messages"
        },
        all: {
          type: "boolean",
          description: "Subscribe to all messages in all channels and DMs"
        },
        mentions_only: {
          type: "boolean",
        ...
        silent_channels: {
          type: "array",
          items: { type: "string" },
          description: "Channel IDs that queue messages but do NOT trigger push notifications. Retrieve with slack_get_queued_messages."
        }
      }
    },
    annotations: {
  ...
  {
    name: "slack_unsubscribe_notifications",
    description: "Remove all notification subscriptions and stop the polling loop.",
    inputSchema: {
      type: "object",
      properties: {}
    },
  ...
  {
    name: "slack_get_queued_messages",
    description: "Retrieve accumulated messages from the notification queue. Messages are marked as fetched after retrieval. " +
      "Use this after receiving a channel notification, or periodically for silent channels.",
    inputSchema: {
      type: "object",
      properties: {
        channel_id: {
          type: "string",
          description: "Optional: filter to a specific channel ID"
        }
      }
    },
    annotations: {
  ...
  {
    name: "slack_get_pipeline_metrics",
    description: "Get notification pipeline metrics: events received, messages enqueued, deduplication stats, notification delivery counts.",
    inputSchema: {
      type: "object",
      properties: {}
    },
